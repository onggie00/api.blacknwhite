...
<?php
    ...
    echo "<h4 style='padding-top:10px;padding-bottom:10px;'>Nomor Kartu Hadiah TV LED</h4>";
    $ctr = 1;
...
<?php
    echo "<div class='kolom_pemenang' style='background-color:#e6e6e6;padding-left:20px;border-radius:5px;margin-top:20px;'>";
    echo "<h4 style='padding-top:10px;padding-bottom:10px;'>Nomor Kartu Hadiah Mesin Cuci</h4>";
    $ctr = 1;
    $k = 1;
    foreach ($pemenang1 as $key => $value) {
        ...
        if ($ctr <=4) {
            echo "<div style='float:left;padding:5px 20px 10px 0px;font-size:13pt;font-weight:bold;'> ".$value->nomer_kartu."</div>";
            $ctr++;
            if($k > count($pemenang1)  && count($pemenang1) == 8){

                echo "<div style='padding:5px 20px 10px 0px;font-size:13pt;font-weight:bold;color:#e6e6e6;'> 123</div>";
            }
            if($k > count($pemenang1)  && count($pemenang1) == 7){
                echo "<div style='padding:5px 20px 10px 0px;font-size:13pt;font-weight:bold;color:#e6e6e6;'> 123</div>";
            }
        }else{
            echo "<div style='padding:5px 20px 10px 0px;font-size:13pt;font-weight:bold;'> ".$value->nomer_kartu."</div>";
...
<div style="clear:both;"></div>
<?php
    echo "<div class='kolom_pemenang' style='background-color:hsl(0, 0%, 90%);padding-left:20px;border-radius:5px;margin-top:20px'>";
    echo "<h4 style='padding-top:10px;padding-bottom:10px;'>Nomor Kartu Hadiah Kulkas</h4>";
    $ctr = 1;
    $p = 1;
    foreach ($pemenang2 as $key => $value) {
        $p++;
        if ($value->nomer_kartu>100 && $value->nomer_kartu<1000) {
            $value->nomer_kartu = "000".$value->nomer_kartu;
        }else if ($value->nomer_kartu>1000 && $value->nomer_kartu<10000) {
        ...
        if ($ctr <=4) {
            ...
            $ctr++;
            if($p > count($pemenang2)  && count($pemenang2) == 8){

                echo "<div style='padding:5px 20px 10px 0px;font-size:13pt;font-weight:bold;color:#e6e6e6;'> 123</div>";
            }
            if($p > count($pemenang2)  && count($pemenang2) == 7){
                echo "<div style='padding:5px 20px 10px 0px;font-size:13pt;font-weight:bold;color:#e6e6e6;'> 123</div>";
            }
        }else{
            echo "<div style='padding:5px 20px 10px 0px;font-size:13pt;font-weight:bold;'> ".$value->nomer_kartu."</div>";
    ...
    ?>
             ...
             <div id="undian_selanjutnya7" class="btn_undian" onclick="elektronik()">
               <a class="btn btn-default" >
               <h5><span ><i class="ion-arrow-forward"> &nbsp;&nbsp; </i></span> Lanjutkan</h5>
              </a>
             </div>
...
<!--Right Section-->
     <section class="right-section right-section-kulkas" style="">
...
//hidden button daftar
 ...
 document.getElementById("undian_selanjutnya"+4).style.display = "none";
//cek daftar
...
if (total_putar == 1) {
 ...
 console.log(cek_kategori);
 if (cek_kategori !=0) {
     document.getElementById("undian_selanjutnya"+4).innerHTML = "Lanjut Hadiah Selanjutnya";
     document.getElementById("undian_selanjutnya"+4).style.width = "auto";
     document.getElementById("undian_selanjutnya"+4).style.padding = "16px";
     document.getElementById("undian_selanjutnya"+4).style.display = "block";
 };
}
...
function motor_umrah(){
 window.location.assign("<?php echo base_url(); ?>undian/motor_umrah");
}
function kulkas(){
...
function elektronik(){
 total_putar++;
 window.location.assign("<?php echo base_url(); ?>undian/elektronik?putaran="+total_putar);
}
function smartphone(){
 total_putar++;
 window.location.assign("<?php echo base_url(); ?>undian/smartphone?putaran="+total_putar);
}
function pulsa(){
