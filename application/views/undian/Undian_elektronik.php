        ...
        <div class="display-table">
          <div class="display-table-cell">
            <div class="main-content">
              <h1 class="title">UNDIAN ELEKTRONIK
                <span id="counter">
...
mempertemukan semua individu yang ingin sukses dan bahagia.</p>
              ...
              <div id="bg-loading" class="hidden" style="width:130px;height:130px;">
                <img src="<?php echo base_url('assets/undian/'); ?>images/Loader.gif" alt="Loading" width="130px" height="130px">
              </div>
              <div id="loading_undian">
              ...
              <div id="play" class="btn_undian" onclick="random()" style="width:auto;">
                ...
                <h5><span ><i class="ion-play"> &nbsp;&nbsp; </i></span> Putar Undian Untuk 20 Nomor</h5>
               </a>
              </div>
...
<!--simpan data hidden-->
      ...
      <img src="<?php echo base_url('assets/undian/'); ?>images/bg-elektronik.png" style="width:100%;height:100%">
...
//close random unique angka
   ...
   function random(){
    $("#bg-loading").attr('style','margin:0 auto;display:block;width:130px;height:130px;');
    if (pencet==0) {
      $(".nomor_undian").attr('style','display:none;');
        document.getElementById("loading_undian").innerHTML = "<h5 style='font-weight:100;text-align:center'>Sistem sedang mengacak seluruh nomor di database untuk mendapatkan 20 pemenang</h5>";
      clearInterval(myVar);
...
//stop
  function stopundian(){
    ...
    console.log(total_putar);
      ...
      if (total_putar == 1) {
        ...
        for (var i = 5; i < 12; i++) {
          nomer_kartu2.push(arr[i]);
        };
        for (var i = 12; i < 20; i++) {
          nomer_kartu3.push(arr[i]);
        };
      }
      else{
        for (var i = 0; i < 5; i++) {
      ...
      console.log(nomer_kartu);
        $.ajax({
            ...
            type : "POST",
            data : { nomer_kartu:nomer_kartu, "category_hadiah" : 7},
            success : function(data) {
                // do something
                console.log(data);
                //window.location = "<?php echo base_url(); ?>undian/daftar_pemenang?category_hadiah=7&putaran="+total_putar;
        ...
        $.ajax({
            url : "<?php echo base_url(); ?>undian/cobainsert",
            type : "POST",
            data : { nomer_kartu:nomer_kartu2, "category_hadiah" : 6},
            success : function(data) {
                // do something
                console.log(data);
                //window.location = "<?php echo base_url(); ?>undian/daftar_pemenang?category_hadiah=7&putaran="+total_putar;
        ...
        $.ajax({
            url : "<?php echo base_url(); ?>undian/cobainsert",
            type : "POST",
            data : { nomer_kartu:nomer_kartu3, "category_hadiah" : 5},
            success : function(data) {
                // do something
                console.log(data);
                window.location = "<?php echo base_url(); ?>undian/daftar_pemenang?category_hadiah=7&putaran="+total_putar;
  ...
  function pemenangundian(){
    window.location.assign("<?php echo base_url(); ?>undian/daftar_pemenang?category_hadiah=7&putaran="+total_putar);
  }
