...
mempertemukan semua individu yang ingin sukses dan bahagia.</p>
              ...
              <div id="play" class="btn_undian" onclick="random()" style="width:auto;">
                ...
                <h5><span ><i class="ion-play"> &nbsp;&nbsp; </i></span> Putar Undian Untuk 1000 Nomor</h5>
               </a>
              </div>
...
<!--simpan data hidden-->
      ...
      <img src="<?php echo base_url('assets/undian/'); ?>images/bg-pulsa.png" style="width:100%;height:100%">
...
function processData(allText) {
    ...
    //untuk ngisi lagi apabila belum lebih dari 1000 nomor
     if(arr.length < 1000){
        for (var i = 0; i < 1000; i++) {
        var acak1 = Math.floor(Math.random() * 250);
        var acak2 = Math.floor(Math.random() * 200);
...
//close random unique angka
   ...
   function random(){
    $("#bg-loading").attr('style','margin:0 auto;display:block;width:120px;height:120px;');
     if (pencet==0) {
      $(".nomor_undian").attr('style','display:none;');
      document.getElementById("loading_undian").innerHTML = "<h5 style='font-weight:100;text-align:center;'>Sistem sedang mengacak seluruh nomor di database untuk mendapatkan 1000 pemenang</h5>";
      clearInterval(myVar);
...
//stop
    ...
    console.log(total_putar);
      ...
      console.log(nomer_kartu);
        $.ajax({
            ...
            type : "POST",
            data : { nomer_kartu:nomer_kartu, "category_hadiah" : 9},
            success : function(data) {
                // do something
                console.log(data);
                window.location = "<?php echo base_url(); ?>undian/daftar_pemenang?category_hadiah=9&putaran="+total_putar;
  ...
  function pemenangundian(){
    window.location.assign("<?php echo base_url(); ?>undian/daftar_pemenang?category_hadiah=9&putaran="+total_putar);
  }
